...
use cgmath::{Point, Vector, EuclideanVector, Point2, Point3, Vector3};

use renderer::{WorkPool, Renderer, Status};
use renderer::algorithm::contribute;
use film::{Film, Sample, Pixel};
use world::World;
use lamp::Surface;
use cameras::Camera;
use tracer::{trace, Color, Light, Bounce, BounceType, RenderContext};
use spatial::kd_tree::{self, KdTree};
use spatial::Dim3;
use utils::{pairs, BatchRange};

pub fn render<W: WorkPool, F: FnMut(Status)>(film: &Film, workers: &mut W, mut on_status: F, renderer: &Renderer, config: &Config, world: &World, camera: &Camera) {
    let num_tiles = film.num_tiles();
    let mut progress = 0;
    let mut rng_maker: XorShiftRng = rand::thread_rng().gen();

    let num_passes = renderer.pixel_samples as usize * config.photon_passes;

    let photon_probability = 1.0 / (renderer.bounces as f64 * config.photon_bounces as f64 * config.photon_passes as f64);

    for pixel_pass in 0..renderer.pixel_samples {
        let status_message = format!("(pass {}/{}): observing the scene", pixel_pass as usize * config.photon_passes, num_passes);
        ...
        progress = 0;
        workers.do_work(film.into_iter().map(|f| (f, rng_maker.gen::<XorShiftRng>())), |(mut tile, mut rng)| {
            ...
            for _ in 0..tile.area() as usize {
                ...
                let bounces = trace(&mut rng, ray, light, world, renderer.bounces, renderer.light_samples);
        ...
        for photon_pass in 0..config.photon_passes {
            ...
            let status_message = format!("(pass {}/{}): shooting photons", photon_pass + pixel_pass as usize * config.photon_passes, num_passes);
            on_status(Status {
                progress: 0,
                message: &status_message
            });
            ...
            workers.do_work(
                BatchRange::new(0..config.photons, 5000).map(|batch| {
                    let mut rng: XorShiftRng = rng_maker.gen();
                    (batch, rng)
                }),
                |(num_rays, mut rng)| {
                    ...
                    for _ in 0..num_rays {
                        let res = world.pick_lamp(&mut rng).and_then(|(lamp, p)| lamp.sample_ray(&mut rng).map(|s| (lamp, p, s)));
                        ...
                        if let Some((lamp, probability, mut ray_sample)) = res {
                            ...
                            let (color, normal) = match ray_sample.surface {
                                Surface::Physical { normal, material } => {
                                    let color = material.get_emission(&mut light, &-ray_sample.ray.direction, &normal, &mut rng);
                                    (color, normal)
                                },
                            ...
                            if let Some(color) = color {
                                ray_sample.ray.origin.add_self_v(&normal.direction.mul_s(0.00001));

                                let mut bounces = trace(&mut rng, ray_sample.ray, light.clone(), world, config.photon_bounces, 0);
                                let p = 1.0 / config.photon_bounces as f64;

                                let incident = bounces.get(0).map(|b| -b.incident).unwrap_or(Vector3::new(0.0, 0.0, 0.0));
            ...
            let status_message = format!("(pass {}/{}): gathering light", photon_pass + pixel_pass as usize * config.photon_passes, num_passes);
            on_status(Status {
                progress: 0,
                message: &status_message
            });
            ...
            workers.do_work(camera_bounces.chunks(5000).map(|b| (b, rng_maker.gen::<XorShiftRng>())), |(bounces, mut rng)| {
                for mut hit in bounces {
                    ...
                    let neighbors: Vec<_> = light_bounces.neighbors(&point, config.radius).collect();
                    ...
                    for neighbor in neighbors {
                        ...
                        if bounce_light.is_white() || neighbor_light.is_white() {
                            let (mut use_additional, wavelength) = if bounce_light.is_white() && neighbor_light.is_white() {
                            ...
                            if weight > 0.0 {
                                weight *= hit.bounce.incident.dot(&-hit.bounce.normal.direction).max(0.0);
                    ...
                    if num_neighbors == 0 {
                        for _ in 0..renderer.spectrum_samples + 1 {
                            let sample = Sample {
                                wavelength: film.sample_wavelength(&mut rng),
                                brightness: 0.0,
                                weight: 1.0 / (renderer.bounces as f64 * config.photon_passes as f64)
                            };
        ...
        film.merge_pixels(camera_bounces.iter().map(|b| (b.position, b.pixel.read().expect("could not read pixel").clone())));
    }
}
...
impl<'a> kd_tree::Element for Arc<LightBounce<'a>> {
    ...
    fn sq_distance(&self, point: &KdPoint) -> f64 {
        self.bounce.normal.origin.sub_p(&point.0).length2()
    }
}
